function right_r=right_main(I,r,meanx,meany)
%% 左侧半圆环积分，步长为5 
    [m,n]=size(I);
    img_incline=zeros(m,n);
     for i=1:m
         for j=meanx:n  % meanx:n %1:n  
               if atan((i-meany)/(j-meanx))*180/pi <10 && atan((i-meany)/(j-meanx))*180/pi >-10
...
%{
     ...
     for i=2:m-1
         for j=3:n-1   % meanx:n %1:n  
             ...
             if img_incline(i,j)==0 && img_incline(i+1,j)~=0 && sun<19000 || img_incline(i+1,j)==0 && img_incline(i,j)~=0 && sun<19000
                    img_incline(i-1,j)=1;  img_incline(i,j-1)=1;img_incline(i,j)=1;
                    sun=sun+1;
              end
         end
     end
...
%}
    %  figure,imshow(img_incline);   
     %%left_r
    I_sum(1)=0; step=1;
    for k=1:3:round(n-meanx) % round((m-meany)/sin(2*pi/9))
       I_sum=0; number=0; I_test=ones(m,n);
        for i=1:m %meany:m
            for j= meanx:n %1:n %meanx:n    1:n
                if  sqrt((i-meany)^2+(j-meanx)^2)<k+5 && k<=sqrt((i-meany)^2+(j-meanx)^2)  &&  atan((i-meany)/(j-meanx))*180/pi <10 && atan((i-meany)/(j-meanx))*180/pi >-10
...
%     figure; hold on; box on;  
%     plot(xq,vq2);
%     plot(index,data(index),'ro');
%     legend('扇环投影曲线','灰度变化最大点')  
    right_r=index+5;