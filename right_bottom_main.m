function right_bottom_r=right_bottom_main(I,r,meanx,meany)
%% 左侧半圆环积分，步长为5 
    [m,n]=size(I);
    img_incline=zeros(m,n);
     for i=meany:m
         for j=meanx:n  % meanx:n %1:n  
               if atan((i-meany)/(j-meanx))*180/pi >20 && atan((i-meany)/(j-meanx))*180/pi<30
       ...
       I_sum=0; number=0; I_test=ones(m,n);
        for i=meany:m
            for j= meanx:n %1:n %meanx:n    1:n
                if  sqrt((i-meany)^2+(j-meanx)^2)<k+5 && k<=sqrt((i-meany)^2+(j-meanx)^2)  &&  atan((i-meany)/(j-meanx))*180/pi>20 && atan((i-meany)/(j-meanx))*180/pi<30
